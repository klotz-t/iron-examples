...
!  REAL(CMISSRP), PARAMETER :: LENGTH=2.0_CMISSRP ! X-direction
  REAL(CMISSRP), PARAMETER :: LENGTH=2_CMISSRP ! X-direction
...
!  REAL(CMISSRP), PARAMETER :: TIME_STOP=300.0_CMISSRP
  REAL(CMISSRP), PARAMETER :: TIME_STOP=10.0_CMISSRP
...
!--------------------------------------------------------------------------------------------------------------------------------
  init_stretch                  = 1.1_CMISSRP
	numberofcontrolloopiterations = 10
!---------------------------------------------------------------------------------------------------------------------------------------
...
!  CALL cmfe_DiagnosticsSetOn(CMFE_FROM_DIAG_TYPE,[1,2,3,4,5],"dignostics",["FIELD_MAPPINGS_CALCULATE"],err)
  ...
  CALL cmfe_OutputSetOn("muscle_tendon",Err)
...
!        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*1.0_CMISSRP,Err) ! To change the initial half-sarcomere length. To create cases similar to Rode. 
        & CMFE_BOUNDARY_CONDITION_FIXED_INCREMENTED,LENGTH*init_stretch,Err) ! To change the initial half-sarcomere length. To create cases similar to Rode. 
    ENDIF
  ENDDO
  ...
  IF(EXPORT_FIELD) THEN
    ...
    CALL cmfe_Fields_NodesExport(Fields,"muscle_tendonExample_M","FORTRAN",Err)
    CALL cmfe_Fields_ElementsExport(Fields,"muscle_tendonExample_M","FORTRAN",Err)
    CALL cmfe_Fields_Finalise(Fields,Err)

    CALL cmfe_Fields_Initialise(Fields,Err)
    CALL cmfe_Fields_Create(RegionFE,Fields,Err)
    CALL cmfe_Fields_NodesExport(Fields,"muscle_tendonExample_FE","FORTRAN",Err)
    CALL cmfe_Fields_ElementsExport(Fields,"muscle_tendonExample_FE","FORTRAN",Err)
    CALL cmfe_Fields_Finalise(Fields,Err)
  ENDIF
...
END PROGRAM muscle_tendonEXAMPLE
